...
export default function EmpreendimentoApp({ emp }: { emp: Empreendimento }) {
  ...
  // --- O CÉREBRO: Descobre qual é a entrada mínima real exigida pela CAIXA baseada na renda
  const minEntradaPermitida = useMemo(() => {
    if (!modelo || rendaFamiliar <= 0) return emp.simulador.entradaMin;
    
    // Roda uma simulação silenciosa tentando financiar com Entrada 0,
    // para ver onde o teto de renda "bate" e corta o financiamento.
    const sim = simular({
    ...
    return Math.max(emp.simulador.entradaMin, Math.ceil(sim.entradaMinimaPRICE));
  }, [modelo, rendaFamiliar, emp.simulador.entradaMin, emp.simulador.prazoMeses, taxaAtual, subsidio, usarSubsidio, emp.mcmv.tetoImovel]);

  // Se a renda informada mudar e a entrada atual ficou proibida, o slider anda sozinho!
  useEffect(() => {
      ...
      <nav style={{ flex: 1, padding: "16px 12px", overflowY: "auto", minHeight: 0 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          {MODULOS.map((mod) => {
            ...
            return (
              <button key={mod.id} onClick={() => { setModuloAtivo(mod.id); onNavigate?.(); }}
                style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 14px", borderRadius: "12px", border: isActive ? "1px solid var(--border-active)" : "1px solid transparent", background: isActive ? "var(--terracota-glow)" : "transparent", cursor: "pointer", transition: "all 150ms ease", textAlign: "left", width: "100%" }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: isActive ? "var(--terracota)" : "rgba(0,0,0,0.3)", flexShrink: 0, transition: "all 150ms ease" }}>
                  {status === "done" && !isActive ? <CheckCircle2 size={15} color="#4ade80" /> : <Icon size={15} color={isActive ? "white" : "var(--gray-mid)"} />}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontWeight: 600, fontSize: 13, color: isActive ? "var(--terracota-light)" : "var(--gray-light)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{mod.label}</p>
                  <p style={{ fontSize: 11, color: "var(--gray-dark)", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{mod.hint}</p>
                </div>
      ...
      <div style={{ padding: "16px 12px 20px", borderTop: "1px solid var(--border-subtle)", flexShrink: 0 }}>
        ...
        {modelo && (
          <div style={{ marginTop: 12, padding: "12px 14px", borderRadius: 12, background: "rgba(0,0,0,0.3)", border: "1px solid var(--border-subtle)" }}>
            ...
            {subsidio > 0 && usarSubsidio && <p style={{ fontSize: 11, color: "#4ade80", marginTop: 4 }}>Subsídio: {formatBRL(subsidio)}</p>}
          </div>
        )}
      </div>
    </div>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside className="fixed left-0 top-0 bottom-0 z-50 flex flex-col lg:hidden" style={{ width: 288, background: "rgba(15,30,22,0.99)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border-subtle)", overflow: "hidden" }} initial={{ x: -288 }} animate={{ x: 0 }} exit={{ x: -288 }} transition={{ type: "spring", damping: 28, stiffness: 320 }}>
              ...
              <div style={{ flex: 1, overflow: "hidden" }}><SidebarContent onNavigate={() => setSidebarOpen(false)} /></div>
            </motion.aside>
          </>
        )}
      </AnimatePresence>
            ...
            <AnimatePresence mode="wait">
              ...
              {moduloAtivo === "renda" && (
                <motion.div key="renda" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ display: "flex", flexDirection: "column", gap: 40 }}>
                  ...
                  <div className="glass-card-nohover" style={{ padding: 48 }}>
                    <SubsidioGauge faixas={emp.mcmv.faixas} onSubsidioChange={handleSubsidioChange} initialRenda={rendaFamiliar} valorImovel={modelo?.valor || 0} tetoMcmv={emp.mcmv.tetoImovel} />
                  </div>
                  {rendaPreenchida && (
              ...
              {moduloAtivo === "simulador" && (
                <motion.div key="simulador" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ display: "flex", flexDirection: "column", gap: 40 }}>
                  ...
                  {subsidio > 0 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-5" style={{ borderLeft: usarSubsidio ? "4px solid #4ade80" : "4px solid #fb923c" }}>
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 style={{ fontWeight: 700, color: "var(--gray-light)", fontSize: 14 }}>Aplicar Subsídio MCMV de {formatBRL(subsidio)}?</h4>
                          <p style={{ fontSize: 12, color: "var(--gray-mid)", marginTop: 4 }}>A Caixa pode zerar o subsídio para compradores solteiros e sem dependentes.</p>
                        </div>
                        <button onClick={() => setUsarSubsidio(!usarSubsidio)} style={{ padding: "8px 16px", borderRadius: 8, fontWeight: 700, fontSize: 12, transition: "all 0.2s", background: usarSubsidio ? "#4ade80" : "rgba(255,255,255,0.1)", color: usarSubsidio ? "#000" : "var(--gray-mid)", border: usarSubsidio ? "none" : "1px solid var(--border-subtle)" }}>
                  ...
                  <div className="glass-card-nohover">
                    <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--gray-mid)", marginBottom: 20 }}>Escolha o Modelo</h3>
                    <ModelSelector modelos={emp.modelos} selected={modeloSelecionado} onSelect={setModeloSelecionado} />
                  </div>

                  {modelo && (
                    <div className="glass-card-nohover">
                      <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--gray-mid)", marginBottom: 20 }}>Defina a Entrada Total</h3>
                      
                      {/* O ALERTA DA TRAVA DE RENDA (MOTOR DE APROVAÇÃO) */}
                      {minEntradaPermitida > emp.simulador.entradaMin && (
                        <div style={{ marginBottom: 20, padding: "14px 16px", borderRadius: 10, background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.25)", display: "flex", alignItems: "flex-start", gap: 12 }}>
                          <Info size={16} color="#ef4444" style={{ flexShrink: 0, marginTop: 2 }} />
                          <p style={{ fontSize: 13, color: "#fca5a5", lineHeight: 1.5 }}>
                            <strong>Trava de Renda da Caixa (Regra dos 30%):</strong> A renda familiar de {formatBRL(rendaFamiliar)} permite uma parcela máxima de <strong>{formatBRL(rendaFamiliar * 0.3)}</strong>. Portanto, o financiamento máximo aprovado é de {formatBRL(resultadoSimulacao?.finLiberadoPRICE || 0)}. A entrada obrigatória subiu para <strong>{formatBRL(minEntradaPermitida)}</strong>.
                          </p>
                        </div>
                      )}
                  ...
                  {modelo && resultadoSimulacao && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                        <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--gray-mid)" }}>Composição da Entrada</h3>
                        <ResultCards valorImovel={modelo.valor} entrada={entrada} subsidio={usarSubsidio ? subsidio : 0} atoPercent={atoPercent} onAtoPercentChange={setAtoPercent} />
                      </div>

                      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                        <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--gray-mid)" }}>Simulador de Financiamento</h3>
                        {resultadoSimulacao.finLiberadoPRICE > 0 ? (
                          <ComparadorSacPrice valorFinanciado={resultadoSimulacao.finLiberadoPRICE} taxaAnual={taxaAtual} prazoMeses={emp.simulador.prazoMeses} rendaFamiliar={rendaFamiliar} />
                        ) : (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px", borderRadius: 12, background: "rgba(0,0,0,0.2)", border: "1px solid var(--border-subtle)" }}>
                    ...
                    <p style={{ fontSize: 12, color: "var(--gray-dark)", lineHeight: 1.6 }}>
                      Simulação para {emp.cidade}-{emp.estado} com taxa nominal de <strong style={{ color: "var(--gray-mid)" }}>{taxaAtual}% a.a.</strong> e prazo de {emp.simulador.prazoMeses} meses. <strong>Os seguros obrigatórios (DFI/MIP) e taxas administrativas já estão embutidos no cálculo das parcelas.</strong> O Laudo de Avaliação exibido é uma estimativa inteligente para viabilizar o financiamento no teto do MCMV. Sujeito à análise de crédito da Caixa Econômica Federal.
                    </p>
                  </div>
              ...
              {moduloAtivo === "obra" && (
                <motion.div key="obra" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ display: "flex", flexDirection: "column", gap: 28 }}>
                  ...
                  {modelo && resultadoSimulacao && resultadoSimulacao.finLiberadoPRICE > 0 ? (
                    <div className="glass-card-nohover">
                      <ObrasEscadaChart valorFinanciado={resultadoSimulacao.finLiberadoPRICE} taxaAnual={taxaAtual} percentuaisPorMes={emp.simulador.percentualObraPorMes} titulo={emp.textos.tituloObra} descricao={emp.textos.descricaoObra} valorLote={modelo.valorLote || 48000} parcelaSAC={resultadoSimulacao?.parcelaSACPrimeira || 0} parcelaPRICE={resultadoSimulacao?.parcelaPricePrimeira || 0} />
                    </div>
                  ) : (
                      ...
                      <div className="glass-card-nohover" style={{ padding: 32 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                          {[
                            ...
                            ...(subsidio > 0 && usarSubsidio ? [["Subsídio MCMV aplicado", formatBRL(subsidio)]] : []),
                            ...
                            <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                              <span style={{ fontSize: 13, color: k.startsWith("  ") ? "var(--gray-dark)" : (k.includes("Laudo") ? "var(--terracota)" : "var(--gray-mid)") }}>{k.trim()}</span>
                              <span style={{ fontSize: 13, fontWeight: 700, color: k.includes("MCMV") || k.includes("Parcela") || k.includes("Aprovado") ? "var(--terracota-light)" : "var(--gray-light)" }}>{v}</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {moduloAtivo === "vitrine" && (
                <motion.div key="vitrine" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ display: "flex", flexDirection: "column", gap: 28 }}>
                  ...
                  <div className="glass-card-nohover"><GaleriaCarousel imagens={emp.vitrine.imagens} plantas={emp.vitrine.plantas} nomeModelo={modelo?.nome || emp.nome} /></div>
                  <div className="glass-card-nohover">
                    <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--gray-mid)", marginBottom: 20 }}>Localização — {emp.cidade}, {emp.estado}</h3>
                    <div style={{ borderRadius: 12, overflow: "hidden", height: 340 }}>
                      <iframe src={`https://maps.google.com/maps?q=${emp.coordenadas.lat},${emp.coordenadas.lng}&z=14&output=embed`} width="100%" height="100%" style={{ border: 0 }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`Mapa ${emp.cidade}`} />
                    </div>
                  </div>
                </motion.div>
              )}
