...
export default function LeadsPublicosPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [empCidade, setEmpCidade] = useState<string>("");
  const [empEstado, setEmpEstado] = useState<string>("");
  ...
  useEffect(() => {
    ...
    const carregarEmpreendimento = async () => {
      try {
        const empQuery = query(collection(db, "empreendimentos"), where("slug", "==", slug));
        const empSnapshot = await getDocs(empQuery);
        
        if (!empSnapshot.empty) {
          const empData = empSnapshot.docs[0].data();
          setEmpNome(empData.nome || "");
          setEmpCidade(empData.cidade || "");
          setEmpEstado(empData.estado || "");
        }
      } catch (error) {
    ...
    const escutarLeads = () => {
      const q = query(collection(db, "leads"), where("empreendimentoId", "==", slug));
      ...
      unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        novosLeads.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        
        setLeads(novosLeads);
        setUltimaAtu(new Date());
        setLoading(false);
        setTimeout(() => setAtualizando(false), 500); // feedback visual de atualização
      }, (error) => {
  ...
  const atualizarStatus = async (leadId: string, status: LeadStatus) => {
    try {
      ...
      // O onSnapshot cuidará de atualizar a interface
    } catch (error) {
        ...
        <div className="container-app">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", flexWrap: "wrap", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
              ...
              <div>
                <p style={{ fontSize: 13, fontWeight: 700, color: "var(--gray-light)" }}>{empNome || slug}</p>
                <div style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--gray-mid)" }}>
                  <MapPin size={11} />
                  <span style={{ fontSize: 11 }}>{empCidade} {empCidade && empEstado ? "·" : ""} {empEstado}</span>
                </div>
              </div>
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "var(--gray-mid)" }}>
              Lista de contatos interessados em {empNome || slug}
            </p>
          </div>
          <div style={{
